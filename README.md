...
>Во второй транзакции были прочитаны данные, которые первая транзакция ещё не зафиксировала. После отката первой транзакции оказалось, что прочитанные данные никогда не существовали в базе.
>Отличие: самый слабый уровень изоляции, допускает чтение «грязных» данных.
...
>Две транзакции независимо изменяли одно и то же значение, и результат одной транзакции был полностью перезаписан другой.
>Отличие: система не защищает от конфликтов обновлений, возможна потеря данных.
...
>Вторая транзакция не смогла прочитать незакоммиченные изменения первой и ожидала завершения транзакции. После отката были прочитаны только зафиксированные данные.
>Отличие: по сравнению с READ UNCOMMITTED, грязное чтение исключено.
...
>В рамках одной транзакции одно и то же чтение дало разные результаты, так как другая транзакция успела закоммитить изменения.
>Отличие: READ COMMITTED защищает только от грязного чтения, но не от изменения данных между чтениями.
...
>Повторное чтение в первой транзакции вернуло те же самые данные, несмотря на попытку изменения во второй транзакции.
>Отличие: в отличие от READ COMMITTED, данные, прочитанные транзакцией, не могут быть изменены другими до её завершения.
...
>Во второй выборке первой транзакции появилось новое логическое «фантомное» значение, добавленное другой транзакцией.
>Отличие: REPEATABLE READ защищает строки, но не защищает диапазоны и агрегатные запросы от фантомов.
...
>Вторая транзакция не смогла изменить данные, попадающие в диапазон запроса первой, до её завершения. Повторный запрос вернул тот же результат.
>Отличие: самый строгий уровень изоляции, полностью исключает фантомное чтение.


# КРАТКИЙ ОТЧЕТ

**Выполнено:** Изучение транзакций и 4 уровней изоляции (READ UNCOMMITTED, READ COMMITTED, REPEATABLE READ, SERIALIZABLE) на реальной БД ломбарда.

**Навыки получены:** 
- Практическая работа с транзакциями (BEGIN/COMMIT/ROLLBACK/SAVEPOINT)
- Настройка уровней изоляции и понимание их различий
- Выявление проблем параллельного доступа: грязное чтение, потерянные изменения, неповторяющееся чтение, фантомы
- Диагностика и решение блокировок в SQL Server

**Выводы:** Каждый уровень изоляции представляет компромисс между производительностью и согласованностью данных. READ COMMITTED - оптимальный выбор для большинства задач, SERIALIZABLE - для критически важных операций, а READ UNCOMMITTED - только для аналитических запросов, где не важна точность.
  </ul>
</div>
...
db.restaurants.updateOne(
  ...
  {
    $set: {
      "working_hours.weekend": "11:00-00:00"
    }
  }
)
...
db.weathers.aggregate([ 
  ...
  {
    $project: { _id: 0, avgTemperature: 1 }
  }
])
...
```
db.weathers.aggregate([
  { $match: { temperature: { $lt: 0 } } }, // все измерения < 0
  {
    $group: {
      _id: { year: "$year", month: "$month", day: "$day" } // группируем по дням
    }
  },
  { $count: "snowyDays" }
])
...
db.weathers.aggregate([
  ...
  },
  {
    $group: {
      _id: null,
      snowDays: {
        $sum: { $cond: [{ $eq: ["$_id.precipitation", "snow"] }, "$count", 0] }
      },
      rainDays: {
        $sum: { $cond: [{ $eq: ["$_id.precipitation", "rain"] }, "$count", 0] }
      }
    }
  },
  {
    $project: {
      ...
      snowDays: 1,
      rainDays: 1,
      difference: { $abs: { $subtract: ["$snowDays", "$rainDays"] } } // модуль
    }
  }
])
...
db.weathers.aggregate([
  {
    $group: {
      _id: { year: "$year", month: "$month", day: "$day" },
      total: { $sum: 1 },
      clearCount: { $sum: { $cond: [{ $eq: ["$code", "CL"] }, 1, 0] } },
      precipitationCount: { $sum: { $cond: [{ $lt: ["$temperature", 0] }, 1, 0] } }
    }
  },
  { $match: { $expr: { $gt: ["$clearCount", { $multiply: ["$total", 0.75] }] } } },
  {
    $group: {
      _id: null,
      probability: { $avg: { $cond: [{ $gt: ["$precipitationCount", 0] }, 1, 0] } }
    }
  },
  { $project: { _id: 0, probability: 1 } }
])
...
```
// 1. Средняя температура до обновления
var avg_before = db.weathers.aggregate([
  { $match: { month: { $in: [12, 1, 2] } } },
  { $group: { _id: null, avgTemp: { $avg: "$temperature" } } }
]).toArray()[0].avgTemp;

// 2. Обновление температуры: +1 градус в нечетные дни зимы
...
// 3. Средняя температура после обновления
var avg_after = db.weathers.aggregate([
  { $match: { month: { $in: [12, 1, 2] } } },
  { $group: { _id: null, avgTemp: { $avg: "$temperature" } } }
]).toArray()[0].avgTemp;
